...
class TestPaneContainer:
    ...
    def test_arrange_side(self, container: PaneContainer) -> None:
        ...
        container.arrange()
        assert container.activityBar.isCollapsible() is True
        assert container.activityBar.isVisible()

    def test_arrange_top(self, container: PaneContainer) -> None:
        container.setAbPosition(ActivityBarPosition.TOP)
        container.arrange()
        assert container.activityBar.isCollapsible() is False
        assert container.activityBar.parent() is container._combined

    def test_top_position_uses_horizontal_bar(self, qtbot: QtBot) -> None:
        c = PaneContainer(default_ab_position="top")
        qtbot.addWidget(c)
        c.addView("a", "A", _label())
        c.addView("b", "B", _label())
        # top position → horizontal ItemBar
        assert isinstance(c.activityBar, ItemBar)
        assert c.activityBar.orientation() == Qt.Orientation.Horizontal
        assert len(c.activityBar.itemIds()) == 2

    def test_side_position_uses_vertical_bar(self, container: PaneContainer) -> None:
        # Default is "side" → vertical ItemBar
        assert isinstance(container.activityBar, ItemBar)
        assert container.activityBar.orientation() == Qt.Orientation.Vertical
